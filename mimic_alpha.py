"""The function `colorAlpha_to_rgb` returns a list of RGB color that mimic a
RGBA on a given background.

The code implements the algorithm from
`this stackoverflow post
<http://stackoverflow.com/questions/2049230/convert-rgba-color-to-rgb?rq=1y%>`_

The code has not been much tested.
A by eye comparison between a pdf image with alpha channel and a few values for
the input color/alpha on white background shows that this approach is 'good
enough'.
...
Licence:
    ...
    This a free sofware and come without any warranty.  It can be freely used,
    modified and redistributed, with the only requirement that the author
    (Francesco Montesano) and the inspiration for the algorithm
...
def _to_rgb(c):
    ...
    """
    if not getattr(c, '__iter__', False):
        # if1: if c is a single element (number of string)
        rgbs = [np.array(cC.to_rgb(c)), ]   # list with 1 RGB numpy array
    else:  # if1, else: if is more that one element
        try:   # try1: check if c is numeric or not
            np.array(c) + 1
        except (TypeError, ValueError):
            # try1: if not numerics is not (only) RGB or RGBA colors
            # convert the list/tuble/array of colors into a list of numpy
            # arrays of RGB
        ...
        else:  # try1: if the colors are all numerics
            ...
            if len(arrcsh) == 1:  # if2: if 1D array given
                ...
                # end if4
            else:  # if2, else: if more dimension
                raise ValueError('The rgb or rgba values must be contained in'
                                 ' a 1D or 2D list or array')
            # end if2
        # end try1
    # end if1
...
def _is_number(s):
    ...
    -------
    bool
        true if c is a number, false otherwise
    """
    try:
...
def _check_alpha(alpha, n):
    """Check if alpha has one or n elements and if they are numberics and
    between 0 and 1
    ...
    alpha: list of numbers
        if all elements numerics and between 0 and 1
    """
    # convert alpha to a flattened array
    alpha = np.array(alpha).flatten()
    if(alpha.size == 1):  # if1: alpha is one element
        if(_is_number(alpha) == False or alpha < 0 or alpha > 1):
            raise ValueError("'alpha' must be a float with value between 0 and"
                             " 1, included")
        else:
            # replicate the alphas len(colors) times
            alpha = [alpha for i in range(n)]
    elif(alpha.size == n):
        # if1, else: if alpha is composed of len(colors) elements
        try:  # check if all alphas are numbers
            alpha+1
        except TypeError:
            raise ValueError("All elements of alpha must be a float with value"
                             " between 0 and 1, included")
        else:
            if((alpha < 0).any() or (alpha > 1).any()):
                raise ValueError("'alpha' must be a float with value between 0"
                                 " and 1, included")
    else:  # if1, else: if none of the previous cases
        raise ValueError("Alpha must have either one element or as many as"
                         " 'colors'")
    # end if1
    return alpha
...
def colorAlpha_to_rgb(colors, alpha, bg='w'):
    """Given a Matplotlib color and a value of alpha, it returns a RGB color
    which mimic the RGBA colors on the given background

    Parameters
    ----------
    colors: Matplotlib color (documentation from
    ...
    # convert the color and save in a list of np arrays
    colors = _to_rgb(colors)
    bg = np.array(cC.to_rgb(bg))   # convert the background

    # check if alpha has 1 or len(colors) elements and return a list of
    # len(color) alpha
